...
const MAX_WALL_YEARS = 100;
const FULL_ROW_COUNT = 10;
const OFFSET_ROW_COUNT = 9;
...
function createWall() {
  ...
  const finalYear = birthYear + MAX_WALL_YEARS - 1;
  ...
  while (year <= finalYear) {
    ...
    const rowSize = isOffsetRow ? OFFSET_ROW_COUNT : FULL_ROW_COUNT;
    const row = document.createElement("div");

    row.className = isOffsetRow ? "brick-row offset-row" : "brick-row";

    for (let i = 0; i < rowSize && year <= finalYear; i++) {
